...
const Navbar = ({ currentView, setCurrentView, mobileOpen, setMobileOpen, searchQuery, setSearchQuery }) => {
  ...
  return (
    <header className="fixed top-0 right-0 w-full md:w-[calc(100%-280px)] h-16 flex justify-between items-center px-container-padding-mobile md:px-container-padding-desktop z-40 backdrop-blur-[12px] border-b border-white/60 dark:border-white/10 bg-surface-container-lowest/40 transition-all duration-300">
      ...
      {/* Global Search Bar */}
      <div className={`flex-1 relative transition-all duration-300 ${searchFocused ? 'max-w-lg md:max-w-xl' : 'max-w-[200px] md:max-w-md'}`}>
        <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-secondary dark:text-secondary-fixed-dim">search</span>
        <input 
          value={searchQuery}
          onChange={(e) => setSearchQuery(e.target.value)}
          onFocus={() => setSearchFocused(true)}
          onBlur={() => setSearchFocused(false)}
          className="w-full glass-input pl-10 pr-4 py-2 rounded-lg text-body-md font-body-md text-on-surface placeholder:text-secondary focus:ring-2 focus:ring-primary/50 transition-all" 
