...
const InventoryList = ({ setCurrentView, searchQuery }) => {
  ...
  const handleOpenAdd = () => {
    ...
    setFormBatch(`B-${new Date().getFullYear()}-${Math.floor(100 + Math.random() * 900)}`);
    setFormExpiry('');
    setFormImage('');
    setFormError('');
    setModalOpen(true);
  ...
  const handleDelete = (id, name) => {
    if (window.confirm(`Are you sure you want to delete batch record for "${name}"?`)) {
      ...
      <div className="glass-panel rounded-2xl overflow-hidden shadow-sm flex flex-col">
        ...
        <div className="p-6 bg-white/20 dark:bg-white/5 border-b border-white/60 dark:border-white/10 flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <h2 className="text-headline-md font-headline-md text-primary">Live Inventory</h2>
            
            {/* Quick stock warning tabs */}
            <div className="flex items-center bg-surface-container-highest/50 rounded-lg p-1">
