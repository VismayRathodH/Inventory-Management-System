...
const Notifications = ({ setCurrentView }) => {
  const { notifications, logs } = useContext(InventoryContext);
  const [filterType, setFilterType] = useState('ALL'); // ALL, LOW_STOCK, EXPIRY

  // Categorize notifications
  // All notifications are considered active until resolved by inventory changes
  const unreadAlerts = notifications;
  
  // Use all notifications (read and unread) for the Expiry Monitor counters
  const allExpired = notifications.filter(n => n.type === 'EXPIRED');
  const allExpiring7 = notifications.filter(n => n.type === 'EXPIRING_SOON' && n.severity === 'HIGH');
  const allExpiring30 = notifications.filter(n => n.type === 'EXPIRING_SOON' && n.severity === 'WARNING');
          ...
          <div className="glass-card rounded-xl p-glass-padding">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="bg-error/5 border border-error/20 rounded-lg p-4 flex flex-col gap-2">
                ...
                <span className="text-display-lg font-display-lg text-error">{allExpired.length}</span>
              ...
              <div className="bg-primary/5 border border-primary/20 rounded-lg p-4 flex flex-col gap-2">
                ...
                <span className="text-display-lg font-display-lg text-primary">{allExpiring7.length}</span>
              ...
              <div className="bg-tertiary/5 border border-tertiary/20 rounded-lg p-4 flex flex-col gap-2">
                ...
                <span className="text-display-lg font-display-lg text-tertiary">{allExpiring30.length}</span>
